...
https://github.com/Efirios/java-explore-with-me/pull/4
...
Основной сервис организован по функциональным пакетам: `user`, `category`, `event`, `request`, `compilation`, `location`, а также `exception` (обработка ошибок) и `util` (общие константы и пагинация).
...
| GET | `/locations/{locId}` | локация по идентификатору |
| GET | `/locations/{locId}/events` | опубликованные события в радиусе локации |
...
| POST | `/admin/locations` | добавить локацию |
| GET | `/admin/locations` | список локаций |
| GET | `/admin/locations/{locId}` | локация по идентификатору |
| PATCH | `/admin/locations/{locId}` | изменить локацию |
| DELETE | `/admin/locations/{locId}` | удалить локацию |

## Дополнительная функциональность: локации

Администратор задаёт локации — города, площадки и т.п. — в виде центра (`lat`, `lon`) и радиуса `radius` (в километрах). Публичный эндпоинт `GET /locations/{locId}/events` возвращает опубликованные события, координаты которых попадают в этот радиус. Расстояние считается по ортодромической формуле прямо в БД (переносимый SQL на функциях `sin`/`cos`/`acos`/`radians`), поэтому фильтрация не тянет лишние данные в приложение. Базовые Postman-тесты фичи лежат в [postman/feature.json](postman/feature.json).
